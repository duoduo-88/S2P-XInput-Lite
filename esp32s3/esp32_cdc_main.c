...
// Task for streaming once per FreeRTOS tick (normally 1 kHz with a 1 ms tick).
...
void tinyusb_cdc_rx_callback(int itf, cdcacm_event_t *event) {
    if (event->type == CDC_ACM_DATA_RX) {
        if (rx_len >= (int)sizeof(rx_buf) - 1) {
            ESP_LOGW(TAG, "Discarding overlong command");
            rx_len = 0;
        }
        ...
        if (ret == ESP_OK && rx_size > 0) {
            ...
            // Process every complete command currently buffered. USB can deliver
            // several newline-delimited commands in one callback.
            char *newline;
            while ((newline = strchr(rx_buf, '\n')) != NULL) {
                *newline = '\0';
                ESP_LOGI(TAG, "Received command: %s", rx_buf);
                
                if (strcmp(rx_buf, "status lite") == 0) {
                    char response[256];
                    snprintf(response, sizeof(response), 
                        "{\"ok\":true, \"cmd\":\"status\", \"version\":\"%s\", \"mode\":\"app\", \"profile\":\"%s\", \"build\":\"%s\", \"ble_channels\": 1, \"usb\": true}\n",
                ...
                else if (strcmp(rx_buf, "ble auto on") == 0) {
                    stream_mode = true;
                }
                else if (strcmp(rx_buf, "ble auto off") == 0 || strcmp(rx_buf, "auto off") == 0) {
                    stream_mode = false;
                }
