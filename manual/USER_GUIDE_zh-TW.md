# S2P-XInput-Lite v0.6.0 使用手冊

本手冊介紹 v0.6.0 的安裝、連線、方案、搖桿、按鍵映射、陀螺儀、震動、HidHide，以及 ESP32-S3 橋接與獨立模式。既有插圖仍適用於未改變的共用介面；v0.6.0 新增的 ESP32 操作以第 8 章為準。
...
v0.6.0 另外提供兩種不需要持續開啟 Windows 程式的獨立輸出：
...
3. 確認 ESP32 已連線且使用支援獨立模式的 `0.14.0-dev` 韌體。
4. 點擊底部 **ESP32 ▼**。
...
> [!NOTE]
> 本手冊以 S2P-XInput-Lite v0.6.0 開發版本為準。正式發佈前若調整介面名稱、參數範圍或連線流程，請以程式內的問號說明與最新發佈說明為準。
