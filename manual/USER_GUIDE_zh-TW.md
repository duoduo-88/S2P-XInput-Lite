...
- **自動辨識獨立模式（實驗性）**：ESP32 先以 USB HID 探測；Android 保持 HID，Windows 發出 Microsoft 描述器要求後會短暫重啟並改列舉為 XInput。

控制器四顆玩家燈會累進顯示實測電量：低電量亮一顆、中低亮兩顆、中高亮三顆、
高電量亮四顆。隨附的 S2P-FW 1.0.5 在獨立模式也能顯示，不需要持續開啟
Windows 程式。
...
2. 程式先讀取目前韌體版本，並與內建的 `S2P-FW 1.0.5` 比對。
...
| 自動辨識獨立模式（實驗性） | 否 | 先 HID 探測，再保持 HID 或切 XInput | 自動選擇 Android HID／Windows XInput |

Auto 模式會先用專屬的 HID 探測身分列舉。Android 會維持 HID；Windows 若要求
Microsoft OS 描述器，ESP32 會軟體重啟一次，再以 XInput 身分重新列舉。由於
Windows 描述器快取、USB Hub 與 Android 裝置實作各異，無法保證每台主機都能
正確辨識；需要最高可靠性時請選固定 XInput 或固定 HID。

部分 Windows 主機可能停留在 USB HID 探測身分，沒有切換為 XInput；請重新插拔
ESP32，或長按 **HOME+X** 3 秒後放開，保存並重啟為固定 PC XInput 模式。

手把已連線時可用以下救援組合鍵：

- 長按 **HOME+X** 3 秒後放開：永久保存並重啟為 PC XInput 模式。
- 長按 **HOME+A** 3 秒後放開：永久保存並重啟為手機 USB HID 模式。
- 長按 **HOME+Y** 5 秒後放開：永久保存 Auto 模式並重新探測主機。
...
3. 確認 ESP32 已連線且使用隨附的 `S2P-FW 1.0.5` 韌體。
4. 點擊底部 **ESP32 ▼**。
5. 選擇 **寫入並啟用 PC XInput 獨立模式**、**寫入並啟用手機 USB HID 模式**，
   或 **寫入並啟用自動辨識獨立模式（實驗性）**。
...
#### 操作範例：在 Windows 啟用 PC XInput 獨立模式

1. 將 ESP32 以 USB 接到要遊玩的 Windows 電腦，並確認主視窗底部已顯示
   ESP32 可用。
2. 選擇要保存到 ESP32 的方案，按 **Save/Apply**，再點擊 **ESP32 ▼**。
3. 選擇 **寫入並啟用 PC XInput 獨立模式**（英文介面為
   **Write and enable PC XInput standalone**），閱讀相容性提示後確認。
4. 等待「寫入完成」訊息；ESP32 會短暫從 USB 消失並重新列舉。這是正常現象，
   此時不需要按 **Restart Connection**。
5. USB 重新出現後，關閉設定視窗也不會停止獨立模式。Windows 應將它辨識為
   XInput 相容控制器；按手把按鍵即可連線至已保存的 ESP32 設定。

選擇 **寫入並啟用手機 USB HID 模式** 可給支援 USB OTG 的 Android 使用；
選擇 **寫入並啟用自動辨識獨立模式（實驗性）** 則由主機嘗試決定 HID 或
XInput。需要最穩定的 Windows 體驗時，請優先選固定的 PC XInput 模式。

> [!WARNING]
> - 寫入後，ESP32 不再是桌面橋接 COM 裝置；桌面連接程式暫時不會處理輸入。
> - 寫入或重新列舉時不要拔除 ESP32、關閉程式或中斷 USB 供電。雖然 A/B 槽會保留
>   上一份已驗證設定，中斷仍會使這次變更未完成。
> - 獨立模式會保存「目前方案」及相容校正資料；Windows 鍵盤／滑鼠、Audio/Mix
>   震動、依程序切換方案與陀螺儀滑鼠不會寫入或執行。
> - Auto 模式在部分 Windows 電腦可能留在 HID 探測身分；重新插拔後若仍發生，
>   長按 **HOME+X** 3 秒再放開，切為固定 PC XInput 模式。
...
- 請在 Windows 電腦上執行此操作；手機 USB HID 主機無法提供切回橋接所需的
  桌面序列通道。
- 選擇 **切回 ESP32 橋接模式**（英文介面為 **Return to ESP32 bridge mode**）
  後，先等待 USB 裝置重新列舉，再等待主程式自動搜尋。不要在第一時間連續點擊
  **Restart Connection**。
- 切回橋接模式不會刪除 ESP32 內最後寫入的獨立模式方案；之後再次選擇獨立模式時，
  仍可重新寫入目前的桌面方案。
- 手把的 HOME+X／HOME+A／HOME+Y 救援組合鍵只能在獨立模式間切換，不能切回
  橋接模式。若無法使用選單，請先重新插拔 ESP32 並開啟 S2P-XInput-Lite，再執行
  **切回 ESP32 橋接模式**。
