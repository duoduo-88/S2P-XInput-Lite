...
USB_SET_FEATURE_MASK_COMMAND = bytes([
    0x0C, 0x91, 0x00, 0x02, 0x00, 0x04,
    # Buttons + sticks + IMU + current + magnetometer. Keep unknown feature
    # bits disabled instead of using the controller-wide 0xFF mask.
    0x00, 0x00, 0xA7, 0x00, 0x00, 0x00,
])
USB_ENABLE_FEATURES_COMMAND = bytes([
...
def initialize_usb_reports():
    ...
    if _write_commands_native_winusb(USB_STARTUP_COMMANDS):
        return True
...
class WiredController:
    ...
    def _run_device(self, entry):
        ...
        try:
            ...
            probe = None
        needs_full_init = not probe or probe[0] != REPORT_ID_COMMON
        self.full_report = not needs_full_init
        initialized = (
            initialize_usb_reports()
            if needs_full_init else True
        )
        if not initialized:
    ...
    def connection_rumble(self):
        def worker():
            for amplitude, duration in ((180, 0.08), (0, 0.10), (180, 0.08), (0, 0.0)):
                self.send_pro_rumble(225, amplitude, 350, amplitude)
                if duration:
                    time.sleep(duration)
        ...
        try:
            while not self._rumble_stop.is_set():
                ...
                if self._rumble_stop.is_set():
                    break
                with self._rumble_lock:
                    data, self._rumble_slot = self._rumble_slot, None
                if data is None:
                    continue
