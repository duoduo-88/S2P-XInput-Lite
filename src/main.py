...
def main():
    ...
    if port is None:
        print()
        print("未偵測到相容的 ESP32。")
        print("請確認 ESP32 已正確連接並已刷入相容韌體。")
        input("\n按 Enter 鍵關閉...")
        return

    print(f"已自動偵測到 ESP32：{port}")
    ...
    # Xbox XInput rumble -> HD Rumble 2 -> ESP32 bridge
    xinput.set_rumble_sender(bridge.send_pro_rumble)
    ...
    bridge.input_callback = on_input

    # 先連接 ESP32
    print(f"正在連接 ESP32：{port}")
            ...
            if COMMAND_PATH.exists():
                try:
                    ...
                    if command == "pin":
                        ...
                        # 只有手把已連線時才發送提醒震動
                        if bridge.connected_channel is not None:
