...
def _curve_settings(y_values, deadzone, outer_deadzone, output_shape, smoothing=0.0):
    settings = {
        "smoothing": f"{float(smoothing):.1f}",
        "deadzone": f"{float(deadzone):.2f}",
        "outer_deadzone": f"{float(outer_deadzone):.2f}",
        "deadzone_compress": "true",
        "outer_deadzone_compress": "true",
        "output_shape": str(int(output_shape)),
        "interpolation": "LINEAR",
    }
    for index, (x_value, y_value) in enumerate(zip(
        (0.0, 0.25, 0.5, 0.75, 1.0), y_values
    )):
        settings[f"point_{index}_x"] = f"{x_value:.3f}"
        settings[f"point_{index}_y"] = f"{float(y_value):.3f}"
    return settings


_NEUTRAL_LEFT = _curve_settings((0.0, 0.25, 0.5, 0.75, 1.0), 0.03, 0.03, 3)
_NEUTRAL_RIGHT = dict(_NEUTRAL_LEFT)
_PRECISION_RIGHT = _curve_settings((0.0, 0.18, 0.42, 0.70, 1.0), 0.03, 0.03, 3)
_RACING_LEFT = _curve_settings((0.0, 0.16, 0.38, 0.68, 1.0), 0.03, 0.03, 0, 0.5)
_ACTION_CURVE = _curve_settings((0.0, 0.30, 0.58, 0.82, 1.0), 0.04, 0.04, 5)
_MUSIC_CURVE = _curve_settings((0.0, 0.25, 0.5, 0.75, 1.0), 0.04, 0.02, 0)

_BALANCED_RUMBLE = {
    "lf_strength": "0.80", "hf_strength": "0.70",
    "lf_to_hf_compensation": "0.20", "hf_to_lf_compensation": "0.10",
    "lf_frequency": "210", "hf_frequency": "325", "max_amplitude": "800",
    "lf_curve": "1.20", "hf_curve": "0.90",
}
_NO_RUMBLE = dict(_BALANCED_RUMBLE, lf_strength="0.00", hf_strength="0.00")
_RACING_RUMBLE = {
    "lf_strength": "0.95", "hf_strength": "0.75",
    "lf_to_hf_compensation": "0.15", "hf_to_lf_compensation": "0.10",
    "lf_frequency": "180", "hf_frequency": "360", "max_amplitude": "900",
    "lf_curve": "0.90", "hf_curve": "1.00",
}
_ACTION_RUMBLE = {
    "lf_strength": "0.90", "hf_strength": "0.85",
    "lf_to_hf_compensation": "0.15", "hf_to_lf_compensation": "0.15",
    "lf_frequency": "210", "hf_frequency": "340", "max_amplitude": "850",
    "lf_curve": "1.00", "hf_curve": "0.90",
}
_MUSIC_RUMBLE = {
    "lf_strength": "0.65", "hf_strength": "0.70",
    "lf_to_hf_compensation": "0.08", "hf_to_lf_compensation": "0.05",
    "lf_frequency": "210", "hf_frequency": "360", "max_amplitude": "700",
    "lf_curve": "1.00", "hf_curve": "0.90",
}

_GAME_AUDIO = {
    "mode": "GAME", "mix_ratio": "0.35", "strength": "0.60",
    "band_1_gain": "1.30", "band_2_gain": "1.15", "band_3_gain": "1.00",
    "band_4_gain": "0.90", "band_5_gain": "0.80", "noise_gate": "0.015",
    "attack_ms": "1", "release_ms": "140", "final_tail_strength": "0.20",
    "final_tail_decay_ms": "350",
}
_MUSIC_AUDIO = {
    "mode": "AUDIO", "mix_ratio": "0.00", "strength": "0.55",
    "band_1_gain": "0.75", "band_2_gain": "0.90", "band_3_gain": "1.10",
    "band_4_gain": "1.20", "band_5_gain": "1.00", "noise_gate": "0.010",
    "attack_ms": "1", "release_ms": "75", "final_tail_strength": "0.00",
    "final_tail_decay_ms": "200",
}

_GYRO_BASE = {
    "activation_mode": "OFF", "activation_button": "ZL",
    "activation_buttons": "ZL", "activation_match": "ANY",
    "target": "RIGHT_STICK", "motion_mode": "CENTER",
    "tilt_axis": "HORIZONTAL", "tilt_recenter_button": "NONE",
    "tilt_max_angle": "35", "tilt_deadzone": "0.80", "tilt_smoothing_ms": "8",
    "stick_sensitivity": "1.50", "mouse_sensitivity": "8.00",
    "deadzone": "0.60", "stick_anti_deadzone": "0",
    "response_curve": "LINEAR", "curve_strength": "5", "smoothing_ms": "5",
    "invert_x": "false", "invert_y": "false", "x_ratio": "1.00",
    "y_ratio": "1.00", "accel_suppression": "70", "adaptive_deadzone": "85",
    "button_freeze_ms": "5", "stabilization_button": "NONE",
    "stabilization_buttons": "NONE", "player_space": "true",
}


def _bundled_profile_overrides():
    """Return deterministic genre starting points; mappings stay user-owned."""
    fps_gyro = dict(
        _GYRO_BASE,
        activation_mode="HOLD",
        stick_sensitivity="1.60",
        deadzone="0.30",
        stick_anti_deadzone="10",
        smoothing_ms="5",
    )
    racing_gyro = dict(
        _GYRO_BASE,
    ...
    return {
        "General": {
            "stick_curve_left": _NEUTRAL_LEFT,
            "stick_curve_right": _NEUTRAL_RIGHT,
            "rumble": _BALANCED_RUMBLE,
            "audio_haptics": _GAME_AUDIO,
            "gyro_mapping": _GYRO_BASE,
        },
        "FPS-COMP": {
            "stick_curve_left": _NEUTRAL_LEFT,
            "stick_curve_right": _PRECISION_RIGHT,
            "rumble": _NO_RUMBLE,
            "audio_haptics": _GAME_AUDIO,
            "gyro_mapping": _GYRO_BASE,
        },
        "FPS-IMM": {
            ...
            "rumble": _NO_RUMBLE,
            "audio_haptics": _GAME_AUDIO,
            "gyro_mapping": fps_gyro,
        },
        "Action": {
            "stick_curve_left": _ACTION_CURVE,
            "stick_curve_right": _ACTION_CURVE,
            "rumble": _ACTION_RUMBLE,
            "audio_haptics": _GAME_AUDIO,
            "gyro_mapping": _GYRO_BASE,
        },
        "Racing": {
            "stick_curve_left": _RACING_LEFT,
            "stick_curve_right": _NEUTRAL_RIGHT,
            "rumble": _RACING_RUMBLE,
            "audio_haptics": _GAME_AUDIO,
            "gyro_mapping": racing_gyro,
        },
        "Rhythm": {
            "stick_curve_left": _MUSIC_CURVE,
            "stick_curve_right": _MUSIC_CURVE,
            "rumble": _MUSIC_RUMBLE,
            "audio_haptics": _MUSIC_AUDIO,
            "gyro_mapping": _GYRO_BASE,
        },
        "Audio": {
            "stick_curve_left": _MUSIC_CURVE,
...
def create_bundled_profiles(config, profile_dir=PROFILE_DIR):
    """Create missing genre presets with canonical default button mappings."""
    existing = {name.casefold() for name in list_profiles(profile_dir)}
    defaults = load_config(DEFAULT_CONFIG_PATH)
    created = []
    for name, section_overrides in _bundled_profile_overrides().items():
        ...
        for section, settings in section_overrides.items():
            for option, value in settings.items():
                profile.set(section, option, str(value))
