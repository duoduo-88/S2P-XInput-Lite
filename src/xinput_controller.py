...
def apply_stick_curve(value, curve_points, interpolation="LINEAR"):
    ...
    if interpolation.strip().upper() == "SMOOTH":
        ...
        # Repeated/reversed X positions represent a vertical or invalid segment;
        # retain the predictable linear behavior for that configuration.
        if widths and all(width > 1e-9 for width in widths):
            secants = [
                (ys[i + 1] - ys[i]) / widths[i]
            ...
            for i, width in enumerate(widths):
                if xs[i] <= magnitude <= xs[i + 1]:
                    t = (magnitude - xs[i]) / width
                    t2 = t * t
                    t3 = t2 * t
                    output = (
                        (2 * t3 - 3 * t2 + 1) * ys[i]
...
class XInputController:
    def __init__(self, config, calibration):
        ...
        self.stick_direction_angle_hysteresis = max(
            0.0,
            min(
                20.0,
                config.getfloat(
        ...
        self.gyro_activation_button = config.get(
            "gyro_mapping", "activation_button", fallback="ZL"
        ).strip().upper()
        if self.gyro_activation_button not in SWITCH_BUTTONS:
            self.gyro_activation_button = "ZL"
        ...
        self.gyro_tilt_smoothing_ms = max(
            ...
            min(150.0, config.getfloat(
                "gyro_mapping", "tilt_smoothing_ms", fallback=30.0
            )),
        )
        ...
        self.gyro_smoothing_ms = max(
            ...
            min(
                100.0,
                config.getfloat(
                    "gyro_mapping", "smoothing_ms", fallback=20.0
                ),
            ),
        )
        ...
        self.gyro_button_freeze_ms = max(
            ...
            min(120.0, config.getfloat(
                "gyro_mapping", "button_freeze_ms", fallback=60.0
            )),
        )
        self.gyro_stabilization_button = config.get(
            "gyro_mapping", "stabilization_button", fallback="NONE"
        ).strip().upper()
        if (
            self.gyro_stabilization_button != "NONE"
            and self.gyro_stabilization_button not in SWITCH_BUTTONS
        ):
            self.gyro_stabilization_button = "NONE"
        ...
        self._gyro_stabilization_button_was_pressed = False
    ...
    def _emit_mouse_wheel(direction):
        delta = 120 if direction == "WHEEL_UP" else -120
        user32.mouse_event(MOUSEEVENTF_WHEEL, 0, 0, ctypes.c_long(delta), 0)
    ...
    def set_audio_rumble(self, lf_level, hf_level):
        ...
        self._publish_rumble_mix()

    def _publish_rumble_mix(self):
        with self._rumble_condition:
        ...
        self._queue_rumble(lf_amp, hf_amp)
    ...
    def _queue_rumble(self, lf_amp, hf_amp):
        with self._rumble_condition:
            self._rumble_state = (
                self.lf_frequency,
                int(lf_amp),
    ...
    def _rumble_dispatch_loop(self):
        """Use one persistent worker instead of one thread per notification."""
        last_sequence = -1
        while True:
            with self._rumble_condition:
                while (
                    ...
                    and (
                        ...
                        or (
                            self._rumble_sequence == last_sequence
                            and self._rumble_state[1] == 0
                            and self._rumble_state[3] == 0
                        )
                    )
                ):
            ...
            is_zero = state[1] == 0 and state[3] == 0
            if not is_zero:
                with self._rumble_condition:
                    if (
                        self._rumble_running
                        ...
                        self._rumble_condition.wait(timeout=0.015)
    ...
    def _get_stick_direction(
        ...
        # Keep the current sector a few degrees beyond its normal boundary.
        # This prevents rapid direction changes while hovering on a boundary.
        direction_centers = {
            "RIGHT": 0.0, "UP_RIGHT": 45.0, "UP": 90.0,
            "UP_LEFT": 135.0, "LEFT": 180.0,
            "DOWN_LEFT": 225.0, "DOWN": 270.0,
            "DOWN_RIGHT": 315.0,
        }
        if current_direction in direction_centers:
            half_sector = 45.0 if mode == "4WAY" else 22.5
            angular_distance = abs(
                (angle - direction_centers[current_direction] + 180.0)
                % 360.0 - 180.0
            )
            if angular_distance <= (
                half_sector + self.stick_direction_angle_hysteresis
            ):
    ...
    def _correct_accelerometer(self, accelerometer):
        values = np.asarray(accelerometer, dtype=np.float64)
        ...
        if accel_bias is not None and accel_matrix is not None:
            corrected = accel_matrix @ (values - accel_bias)
        else:
            ...
            corrected = values / gravity_scale
        magnitude = float(np.linalg.norm(corrected))
        if not 0.25 <= magnitude <= 2.5:
            return None
        return corrected
    ...
    def set_magnetometer_calibration(self, bias, scale=None, matrix=None):
        ...
        if len(bias_values) != 3 or any(
            not math.isfinite(value) or abs(value) > 32768.0
    ...
    def get_magnetometer_calibration_status(self):
        with self._gyro_calibration_lock:
            ...
            progress = int(round(95.0 * min(coverage, elapsed / 12.0)))
    ...
    def _update_gyro_bias(self, gyroscope, accelerometer):
        """Learn stationary sensor bias while gyro mapping is inactive."""
        raw = tuple(float(value) for value in gyroscope)
        # Switch 2 reports observed through the ESP32 feature stream use about
        # 4096 LSB/g, while the normalized/native path used by Switch2Connect
        # can use about 16384 LSB/g.  Accept either scale, but still reject
        # missing all-zero data and implausible motion during bias learning.
        accel_plausible = self._gyro_accel_is_plausible(accelerometer)
        delta_small = False
        if self._gyro_last_raw is not None:
            delta_small = max(
                abs(raw[index] - self._gyro_last_raw[index])
                for index in range(3)
            ) <= 12.0
        self._gyro_last_raw = raw

        if accel_plausible and delta_small:
            self._gyro_stationary_samples += 1
        else:
            self._gyro_stationary_samples = 0
            return
        if self._gyro_stationary_samples < 8:
            return
    ...
    def _correct_magnetometer(self, magnetometer):
        ...
        if getattr(self, "_mag_matrix", None) is not None:
            try:
                return apply_matrix_calibration(
                    magnetometer, self._mag_bias, self._mag_matrix
                )
            except ValueError:
                return None
        return tuple(
            (float(magnetometer[index]) - self._mag_bias[index])
            * getattr(self, "_mag_scale", (1.0, 1.0, 1.0))[index]
            for index in range(3)
        )
    ...
    def _update_nine_axis_orientation(
        ...
        accel = self._correct_accelerometer(accelerometer)
        if dt <= 0.0 or accel is None:
            return self._nine_axis_orientation if self._nine_axis_has_magnetometer else None

        gyro = np.asarray(
            [
                (float(gyroscope[index]) - self._gyro_bias[index])
                / 14.285714
                for index in range(3)
            ],
            dtype=np.float64,
        )
        # During fast rotation or non-gravity acceleration, progressively
        # replace the measured acceleration with the AHRS-predicted gravity.
        # This preserves low-speed leveling while preventing a throw, shake,
        # or centripetal force from pulling the Tilt orientation sideways.
        accel_suppression = getattr(self, "gyro_accel_suppression", 0.0)
        if accel_suppression > 0.0:
            gravity = np.asarray(self._ahrs.gravity, dtype=np.float64)
            gravity_magnitude = float(np.linalg.norm(gravity))
            accel_magnitude = float(np.linalg.norm(accel))
            if 0.5 <= gravity_magnitude <= 1.5 and accel_magnitude > 1e-9:
                angular_speed = float(np.linalg.norm(gyro))
                ...
                magnitude_factor = min(
                    1.0, abs(accel_magnitude - 1.0) / 0.12
                )
                blend = accel_suppression * max(
                    speed_factor, magnitude_factor
                )
                accel = accel * (1.0 - blend) + gravity * blend
    ...
    def _get_gyro_output(self, state, dt):
        ...
        mask = SWITCH_BUTTONS.get(self.gyro_activation_button)
        trigger_pressed = bool(mask is not None and state.buttons & mask)
        trigger_changed = trigger_pressed != self._gyro_trigger_was_pressed
        ...
        stabilization_mask = SWITCH_BUTTONS.get(
            getattr(self, "gyro_stabilization_button", "NONE")
        )
        stabilization_pressed = bool(
            stabilization_mask is not None
            and state.buttons & stabilization_mask
        )
        stabilization_changed = (
            stabilization_pressed
            != getattr(
                self, "_gyro_stabilization_button_was_pressed", False
            )
        )
        self._gyro_stabilization_button_was_pressed = stabilization_pressed
        if (
            self.gyro_activation_mode != "OFF"
        ...
        if self._process_gyro_calibration(gyroscope, accelerometer):
            ...
            self._tilt_neutral_quaternion = None
            return 0.0, 0.0, 0.0, 0.0
        # Nine-axis fusion is needed only by Tilt-to-stick mapping.  Center
        # mode uses angular velocity directly, and OFF should not spend CPU on
        # an output path that cannot be observed.
        ...
        if not active:
            self._update_gyro_bias(gyroscope, accelerometer)
            if tilt_tracking_enabled:
        ...
        if self.gyro_motion_mode == "TILT" and self.gyro_target != "MOUSE":
            ...
            raw_gx, _raw_gy, raw_gz = gyroscope
            # Pro Controller 2 uses approximately 14.285714 LSB per degree/second.
            horizontal = -(float(raw_gz) - self._gyro_bias[2]) / 14.285714
            vertical = (float(raw_gx) - self._gyro_bias[0]) / 14.285714
        ...
        return (
            smoothed_x * stick_scale,
            smoothed_y * stick_scale,
            0.0,
            0.0,
        )
    ...
    def update(self, state):
        ...
        for switch_name, target in self.mapping.items():
            ...
            if target.startswith("MOUSE:"):
                candidate = target[len("MOUSE:"):].strip().upper()
                if candidate in MOUSE_BUTTON_FLAGS:
                    mouse_action = candidate
            previous_mouse_action = self._active_mouse_buttons.get(switch_name)
            source = "BUTTON:" + switch_name
            current_mouse_action = mouse_action if is_pressed else None
            if (
        ...
        for side, direction in (
            ...
            # 滾輪是瞬時事件：只在搖桿第一次進入該映射方向時
            # 送出一格，保持同一方向時不隨輸入回報頻率連發。
        ...
        if left_to_mouse:
            mouse_dx += lx * self.stick_mouse_speed * dt
            mouse_dy += -ly * self.stick_mouse_speed * dt
        if right_to_mouse:
            mouse_dx += rx * self.stick_mouse_speed * dt
            mouse_dy += -ry * self.stick_mouse_speed * dt
