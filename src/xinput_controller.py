...
class XInputController:
    ...
    def __init__(self, config, calibration):
        ...
        self.gyro_smoothing_ms = max(
            ...
            min(
                100.0,
                config.getfloat(
                    "gyro_mapping", "smoothing_ms", fallback=15.0
                ),
            ),
        )
        ...
        self.gyro_button_freeze_ms = max(
            ...
            min(120.0, config.getfloat(
                "gyro_mapping", "button_freeze_ms", fallback=35.0
            )),
        )
        ...
        self.hf_frequency = max(
            1, min(511, config.getint("rumble", "hf_frequency", fallback=481))
        )
        ...
        self.final_tail_decay_ms = max(
            ...
            min(
                2000.0,
                config.getfloat(
                    "audio_haptics", "final_tail_decay_ms", fallback=350.0
                ),
            ),
        )
    ...
    def _queue_rumble(self, lf_amp, hf_amp, force_zero=False):
        with self._rumble_condition:
            self._rumble_state = (
                self.lf_frequency,
                int(lf_amp),
    ...
    def _update_impact_state(self, gyroscope, accelerometer, dt):
        ...
        now = time.perf_counter()
        raw_gyro = np.asarray(gyroscope, dtype=np.float64)
        raw_accel = np.asarray(accelerometer, dtype=np.float64)
        accel_magnitude = float(np.linalg.norm(raw_accel))
        ...
        if gravity_scale is None:
            ...
            if abs(accel_magnitude - gravity_scale) <= gravity_scale * 0.35:
                ...
                return False
        accel_g = raw_accel / gravity_scale
        if dt <= 0.0 or dt > 0.10 or not np.all(np.isfinite(accel_g)):
        ...
        filtered_accel = (
            accel_g if previous_accel is None
            else previous_accel + (accel_g - previous_accel) * alpha
        )
        ...
        jerk = float(np.linalg.norm(filtered_accel - previous_accel)) / dt
        gyro_acceleration = (
            float(np.linalg.norm(raw_gyro - previous_gyro)) / 14.285714 / dt
        )
        magnitude_error = abs(float(np.linalg.norm(filtered_accel)) - 1.0)
    ...
    def _update_gyro_bias(self, gyroscope, accelerometer, magnetometer=None):
        ...
        if candidate:
            ...
            accel_stable = (
                ...
                and float(np.dot(accel_reference, accel_direction)) >= math.cos(math.radians(1.5))
            )
            ...
            mag_stable = (
                ...
                or (
                    ...
                    and float(np.dot(mag_reference, mag_direction)) >= math.cos(math.radians(2.0))
                )
            )
    ...
    def _gravity_aware_aim_axes(self, gyroscope, dt):
        ...
        legacy = (-float(raw_rates[2]), float(raw_rates[0]))
        ...
        if self._aim_gravity_sign is None:
            ...
            if now - self._aim_pose_ready_since < self.AIM_POSE_SETTLE_SECONDS:
                return legacy
            self._aim_gravity_sign = 1.0 if gravity[2] >= 0.0 else -1.0
        gravity *= self._aim_gravity_sign

        vertical_axis = np.asarray((1.0, 0.0, 0.0), dtype=np.float64)
        vertical_axis -= gravity * float(np.dot(vertical_axis, gravity))
        vertical_norm = float(np.linalg.norm(vertical_axis))
        if vertical_norm <= 0.15:
            return legacy
        vertical_axis /= vertical_norm
        transformed = (
            -float(np.dot(raw_rates, gravity)),
            float(np.dot(raw_rates, vertical_axis)),
        )
