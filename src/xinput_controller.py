...
def _compile_stick_direction_settings(raw_sides):
    ...
    for side in ("LEFT", "RIGHT"):
        ...
        mode = _normalize_stick_direction_mode(
            raw.get("mode", "4WAY"), side
        )
        ...
        result[side] = {
            ...
            "analog_direction": validate_stick_analog_direction(
...
# Windows 對方向鍵、導覽鍵與 Windows 鍵使用 extended-key 掃描碼。
# 僅媒體鍵帶此旗標時，部分遊戲／Raw Input 路徑可能無法穩定辨識。
EXTENDED_VK_KEYS = (
    set(range(0x21, 0x29))
    | {0x2D, 0x2E, 0x5B, 0x5C}
    | set(range(0xAD, 0xB4))
...
class XInputController:
    ...
    def __init__(
        ...
        for side in (
            ...
            for setting_name, _setting_value in config.items(section_name):
                try:
                    validate_stick_setting_key(setting_name)
                except ValueError as exc:
                    raise ValueError(
            ...
            try:
                ...
                raise ValueError(
                    _tr(
                        f"{side} 線性輸入方向無效：{exc}",
                        f"Invalid {side} linear input direction: {exc}",
                    )
                ) from exc
        ...
        # }
        #
        # 組合層避免同一來源重複取得；鍵碼層則處理不同組合
        # 共用 Ctrl／Shift／Alt／Win 或一般按鍵的情況。
        # 只有最後一個鍵碼來源放開時，才真正送出 KeyUp。

        self._keyboard_combo_sources = {}
        self._keyboard_key_sources = {}
            ...
            if not key_sources:
                user32.keybd_event(
                    ...
                    (
                        ...
                        if vk_code in EXTENDED_VK_KEYS
                        else 0
                    ),
                    0,
                )
    ...
    def _release_keyboard_combo_source(
        ...
        for vk_code in reversed(
            parse_keyboard_combo(
                combo_text
            )
        ):
            key_sources = (
            ...
            self._keyboard_key_sources.pop(
                vk_code,
                None,
            )
    ...
    def release_all_keyboard_buttons(self):
        ...
        # =========================
        # 釋放每個目前保持的實體鍵碼一次。
        # dict 保留首次取得順序，反向釋放通常會先放一般鍵，
        # 最後才放 Ctrl／Shift／Alt／Win。
        # =========================
        for vk_code in reversed(
            list(
                self._keyboard_key_sources.keys()
            )
        ):
            user32.keybd_event(
                vk_code,
        ...
        # 清空統一來源追蹤
        self._keyboard_key_sources.clear()
    ...
    def _linear_trigger_amount(
        ...
        calibrated_center = calibration["center"]
        center = calibrated_center
        maximum = calibration["max"]
        minimum = calibration["min"]
        if center_override is not None:
            center = center_override
            # Preserve the factory/user-calibrated physical endpoints when a
            # temporary neutral capture slightly shifts the working center.
            maximum = tuple(
                max(
                    1.0,
