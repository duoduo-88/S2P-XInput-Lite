...
class ConfigGUI:
    ...
    def toggle_stick_settings_zoom(self):
        ...
        if self.stick_settings_zoomed:
            ...
            # =========================
            #
            # 放大模式會改寫 scroll Canvas 的 viewport 與內嵌視窗寬度。
            # 720p / 768p 返回時若只恢復 root geometry，Canvas 仍會保留
            # 放大模式的窄寬度，造成正常內容被裁切。先依完整內容重新
            # 計算捲動容器，再恢復進入放大前保存的位置與尺寸。
    ...
    def reset_gyro_mapping(self):
        ...
        self.gyro_tilt_smoothing_var.set("8")
