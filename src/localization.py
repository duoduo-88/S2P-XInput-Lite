...
EN_TEXT = {
    ...
    "感度曲線": "Sens. Curve",
    "還原目前陀螺儀模式": "Restore Current Gyro Mode",
    "還原曲線": "Reset Curve",
    "動態": "Dynamic",
    "後段加速": "Late Boost",
    "前段加速": "Early Boost",
    ...
    "改變低頻震動的觸感。數值不同，震動可能變得較沉、較硬或較弱。\n\n可設定範圍：1 ～ 511\n這是控制器使用的命令值，不是直接的 Hz。":
        "Changes the feel of LF rumble. Different values may feel deeper, harder, or weaker.\n\nCommand range: 1–511\nThis is a controller command value, not a direct Hz value.",
    "改變高頻震動的觸感。數值不同，震動可能變得較厚、較尖或產生較明顯聲音。\n\n可設定範圍：1 ～ 511\n這是控制器使用的命令值，不是直接的 Hz。":
        "Changes the feel of HF rumble. Different values may feel thicker, sharper, or produce more audible sound.\n\nCommand range: 1–511\nThis is a controller command value, not a direct Hz value.",
    "限制低頻與高頻最多能震多強，相當於整體震動的安全上限。\n\n設定範圍：0 ～ 1023\n數值越高，最強震動越大；接近上限時可能增加異音、共振與硬體負擔。":
        "Limits how strong LF and HF rumble can become, acting as the overall output ceiling.\n\nRange: 0–1023\nHigher values allow stronger peak rumble; values near the limit may increase noise, resonance, and hardware load.",
    ...
    "只平滑音訊分析結果的下降速度。\n\n降低：停止更乾脆\n提高：下降更平順，但會拖得較久。":
        "Smooths only the falling audio-analysis envelope.\n\nLower values stop more cleanly; higher values fall more smoothly but linger longer.",
    ...
    "拖曳五個控制點，調整低頻到高頻的音訊增益。\n雙擊控制點，可還原目前音訊模式的該頻段預設。\n1.00＝不額外放大或縮小。\n只影響音訊與混合模式，不影響純遊戲震動。":
        "Drag the five points to adjust audio gain from low to high.\nDouble-click a point to restore that band for the current audio mode.\n1.00 applies no extra boost or reduction.\nOnly Audio and Mix modes are affected; Game mode is unchanged.",
