...
[v0.7.9 release notes](RELEASE_NOTES_v0.7.9.md) ·
...
- Battery percentage estimated from a full measured discharge curve, with a
  shared four-level battery display on the UI and controller player LEDs
...
- Before flashing, the application reads the connected ESP32 firmware version and compares it with the bundled `S2P-FW 1.0.5`. After a successful flash it waits for USB re-enumeration, starts the desktop connector automatically, and searches for the controller again.
...
- The ESP32 connection requires compatible firmware. Standalone profile
  ...
  bundled `S2P-FW 1.0.5`; older S2P builds and upstream firmware cannot provide
