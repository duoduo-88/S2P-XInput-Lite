...
- 可將相容設定檔寫入 ESP32，讓它不開啟 Windows 程式也能固定提供 PC XInput、
  標準手機 USB HID，或以實驗性 Auto 模式辨識 Windows／Android；並可用手把
  組合鍵救援切換模式
...
v0.7.11 的主視窗底部已將原本的「**管理方案**」入口改為「**匯入設定**」，
用來開始完整的安裝資料夾之間設定遷移。

![主視窗中的匯入設定按鈕](manual/assets/annotated/16-import-settings-button.png)
...
- 自動辨識獨立模式屬測試功能。部分 Windows 主機可能停留在 USB HID 探測身分，沒有切換為 XInput；請重新插拔 ESP32，或長按 **HOME+X** 3 秒後放開，保存並重啟為固定 PC XInput 模式。
- ESP32 連線需要相容韌體。寫入獨立模式設定檔、直接 USB 控制器輸出及
  診斷頁需要隨附的 `S2P-FW 1.0.5`；舊版 S2P 韌體與上游韌體無法提供完整
...
- 刷寫前會讀取目前 ESP32 韌體版本並與內建的 `S2P-FW 1.0.5` 比對。刷寫成功後，程式會等待 USB 重新列舉，自動啟動桌面連接並重新搜尋手把。
