...
<p align="center">
  <img src="image/GUI.gif" alt="S2P-XInput-Lite 設定介面" width="46%">
  <img src="image/test.gif" alt="S2P-XInput-Lite 手把測試" width="46%">
</p>
...
- 設定中的 `0`～`1023` 是協議振幅欄位的可用範圍，不代表已公開認證的硬體安全連續輸出上限。本程式的系統預設最大振幅為 `800`，約為欄位上限的 78%（概略可視為 80%），在保留震動表現的同時降低過強輸出造成異音的可能。建議依個別手把狀況及個人偏好調整；若出現撞擊聲或其他異音，請適度降低最大振幅。
